...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    if (category) {
      emails = await elasticsearchService.getEmailsByCategory(category as string);
      // Apply account and folder filters if specified
      if (accountEmail) {
        emails = emails.filter((email: any) => email.accountEmail === accountEmail);
      }
      if (folder) {
        emails = emails.filter((email: any) => email.folder === folder);
      }
    } else if (accountEmail && folder) {
